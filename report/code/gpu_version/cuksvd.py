...
def ksvd(max_iter,n_components,transform_n_nonzero_coefs,X):
    ...
    # replace unuse atoms
    atomsize = X.shape[0]  
    atomnum =  n_components
    batchsize = 2048
    sparsenum = transform_n_nonzero_coefs
    signum = X.shape[1]
    blocksize = 128
    ...
    gamma = np.zeros((n_components,X.shape[1])).astype(np.float32)
    erec = np.zeros(max_iter)
    for ite in range(max_iter):
        print("The {}-th iteration is running now\n".format(ite))
#        for i in range(X.shape[1]):
#            gamma[:,i] = omp(D,X[:,i],ncoef=3)
        gamma,_ = cuomp.batchomp(D,X,gamma,signum,batchsize)   
        print gamma.shape         
