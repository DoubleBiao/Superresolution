...
disp('The experiment corresponds to the results from Table 1 in the reference paper.');
...
for d=7    
    ...
    if 0 %exist([mat_file '.mat'],'file')
        ...
        startt = tic;
        conf = learn_dict(conf, load_images(...      % 这个是training函数的入口，我们主要的精力都放在这个上面         
            glob('CVPR08-SR/Data/Training', '*.bmp') ...       %我们只需要实现一个SR算法，不需要和其它函数对比之类的，所以这里面的很多东西可以省去
            ), dict_sizes(d));       
